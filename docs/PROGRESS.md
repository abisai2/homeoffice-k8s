...
- **Next action:** **P5** — author `scripts/release.sh` + `VERSION` + `CHANGELOG.md` (single-pin discipline: bumps BOTH `platform-appset.yaml` and `root-app.yaml` `targetRevision` in lockstep; rejects non-SemVer; exit-verify = dry-run bumps exactly 2 pins). Then P6 (KSOPS secret-flow ergonomics), P7 (platform stack: cert-manager → gateway → longhorn → cnpg → velero → etcd-backup → authentik). Note the GitHub remote is empty — the `root` app stays Unknown until the `build` branch is pushed + `v0.1.0` tagged at P7.9 (P7.9 advances the live pin and Argo syncs the platform). _(The one-time live gateway-api catch-up — tlsroutes CRD + operator restart + GatewayClass — was completed 2026-06-04; see checkpoint above.)_
