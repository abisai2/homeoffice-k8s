...
- **Next action:** **P6** — KSOPS secret-flow ergonomics: per-app `secret-generator.yaml` (KSOPS) pattern + `.example` siblings + Taskfile `sops:*` helpers (exit-verify: KSOPS generator renders locally where possible; documented). Then P7 (platform stack: cert-manager → gateway → longhorn → cnpg → velero → etcd-backup → authentik), cutting `v0.1.0` at P7.9 via tag + `argocd app set`. **Reminder for every P7 helm app:** grep the pulled chart templates for `.Capabilities.APIVersions.Has` and force the matching `*.create:"true"` in values (offline render drops capability-gated objects — see the Cilium GatewayClass fix). Note the GitHub remote is empty — the `root` app stays Unknown until the `build` branch is pushed + `v0.1.0` tagged at P7.9.
